...
int l = 0, slp = 70000, flagdif = 10, godmode = 0;
int main() {
	clear();
	refresh();
	curs_set(0);
...
void settings() {
	...
	set_item_userptr(my_items[1], plives);
	set_item_userptr(my_items[2], godmod);
	set_item_userptr(my_items[3], clearscores);
	set_item_userptr(my_items[4], twoplayer);
...
void instruct() {
	...
	mvwprintw(mainwin, LINES / 2 - 5, 35, "YOU HAVE 5 LIVES INITIALLY, WHICH GREENUCES IF ANY OF THE SEGFAULTS COLLIDES YOU OR YOUR WALL.");
	mvwprintw(mainwin, LINES / 2 - 3, 35, "YOU LOSE THE GAME WHEN LIFE IS ZERO.YOU CAN ALSO INCREASE OR DECREASE PLAYERS LIFE.");
